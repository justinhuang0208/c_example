...
int mon_total(int, int, float); 	/* 函式宣告mon_total函式採每月計息一次 */
...
int main(int argc, char *argv[])
{
		...
		printf("\n");
		printf(" *** 每年計息一次，%d年後領回本利和：%d",
		years,yr_total(money, years, per_rate));
		printf("\n\n");
		printf(" *** 每月計息一次，%d年後領回本利和：%d",
		years,mon_total(money, years, per_rate));
		printf("\n\n");
   	system("PAUSE");	
   	return 0;
}
